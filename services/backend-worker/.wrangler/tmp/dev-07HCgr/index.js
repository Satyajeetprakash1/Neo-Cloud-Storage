...
// .wrangler/tmp/bundle-3ZU9kk/checked-fetch.js
...
// .wrangler/tmp/bundle-3ZU9kk/strip-cf-connecting-ip-header.js
...
__name(RealtimePostgresFilterBuilder, "RealtimePostgresFilterBuilder");
...
var RealtimeChannel = class {
   ...
   * @internal
   */
  static isSamePostgresFilter(a2, b) {
    var _a, _b, _c, _d;
    const selectA = (_b = (_a = a2 === null || a2 === void 0 ? void 0 : a2.select) === null || _a === void 0 ? void 0 : _a.join()) !== null && _b !== void 0 ? _b : void 0;
    const selectB = (_d = (_c = b === null || b === void 0 ? void 0 : b.select) === null || _c === void 0 ? void 0 : _c.join()) !== null && _d !== void 0 ? _d : void 0;
    return (a2 === null || a2 === void 0 ? void 0 : a2.event) === (b === null || b === void 0 ? void 0 : b.event) && RealtimeChannel.isFilterValueEqual(a2 === null || a2 === void 0 ? void 0 : a2.schema, b === null || b === void 0 ? void 0 : b.schema) && RealtimeChannel.isFilterValueEqual(a2 === null || a2 === void 0 ? void 0 : a2.table, b === null || b === void 0 ? void 0 : b.table) && RealtimeChannel.isFilterValueEqual(a2 === null || a2 === void 0 ? void 0 : a2.filter, b === null || b === void 0 ? void 0 : b.filter) && selectA === selectB;
  }
  /** @internal */
...
// src/index.ts
var router = c();
var getSupabase = /* @__PURE__ */ __name((env) => {
  return createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY, {
    auth: { persistSession: false }
  });
}, "getSupabase");
router.post("/api/v1/auth/verify", async (request, env) => {
  const authHeader = request.headers.get("Authorization");
  if (!authHeader)
    return s(401, "Missing Authorization header");
  const supabase = getSupabase(env);
  const { data: { user }, error: authError } = await supabase.auth.getUser(authHeader.replace("Bearer ", ""));
  if (authError || !user) {
    return s(401, "Invalid token");
  }
  return r({ valid: true, user_id: user.id });
});
router.post("/api/v1/upload/init", async (request, env) => {
  const { chunks, nodeId } = await request.json();
  if (!chunks || !Array.isArray(chunks))
    return s(400, "Invalid chunks array");
  const supabase = getSupabase(env);
  const { data: existingChunks, error: dbError } = await supabase.from("global_chunks").select("chunk_hash, r2_object_key").in("chunk_hash", chunks);
  if (dbError)
    return s(500, dbError.message);
  const existingHashMap = new Map(existingChunks?.map((c2) => [c2.chunk_hash, c2.r2_object_key]) || []);
  const missingChunks = [];
  const existingList = [];
...
router.put("/api/v1/upload/chunk/:hash", async (request, env) => {
  const hash = request.params.hash;
  if (!request.body)
    return s(400, "Missing request body");
  await env.STORAGE_BUCKET.put(`chunks/${hash}`, request.body, {
    httpMetadata: { contentType: "application/octet-stream" }
  });
  return r({ success: true, hash });
});
router.post("/api/v1/upload/complete", async (request, env) => {
  const { nodeId, chunks, sizeBytes } = await request.json();
  const supabase = getSupabase(env);
  return r({ success: true, nodeId });
});
router.get("/api/v1/download/:nodeId", async (request, env) => {
  const nodeId = request.params.nodeId;
  const supabase = getSupabase(env);
  const { data: fileChunks, error: err } = await supabase.from("file_chunks").select("chunk_index, chunk_hash, global_chunks(r2_object_key)").eq("node_id", nodeId).order("chunk_index", { ascending: true });
  if (err)
    return s(500, err.message);
  const downloadUrls = fileChunks?.map((fc) => ({
    index: fc.chunk_index,
    hash: fc.chunk_hash,
    url: `/api/v1/download/chunk/${fc.global_chunks.r2_object_key}`
  }));
  return r({ chunks: downloadUrls, encrypted_fek: "mock_encrypted_fek" });
});
router.get("/api/v1/download/chunk/:r2key", async (request, env) => {
  const r2key = request.params.r2key;
  const object = await env.STORAGE_BUCKET.get(r2key);
  if (!object)
    return s(404, "Chunk not found");
...
var src_default = {
  ...
  async scheduled(event, env, ctx) {
    ctx.waitUntil((async () => {
      const supabase = getSupabase(env);
      const { data: toDelete, error: fetchErr } = await supabase.from("deleted_chunks_queue").select("id, r2_object_key").limit(100);
      ...
      await supabase.from("deleted_chunks_queue").delete().in("id", queueIds);
...
// .wrangler/tmp/bundle-3ZU9kk/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3ZU9kk/middleware-loader.entry.ts
